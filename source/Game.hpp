...
void RunGame()
{
  ...
  bool screenSize;
  ifstream resolutoinFile("resolutoin.txt");
  ...
  anim2.create("explode",bullet_t,27,7,18,18,4,0.01f,29,false);

  AnimationManager anim3;
  anim3.create("move",enemy_t,1,0,50,50,2,0.002f,51);
  anim3.create("dead",enemy_t,104,0,50,50,2,0.002f,51,0);
  ...
  for (unsigned int i=0;i < e.size();i++)
    entities.push_back(new ENEMY(anim3, static_cast<int>(e[i].rect.left), static_cast<int>(e[i].rect.top)) );

  e = lvl.GetObjects("MovingPlatform");
  for (unsigned int i=0;i < e.size();i++)
    entities.push_back(new MovingPlatform(anim4, static_cast<int>(e[i].rect.left), static_cast<int>(e[i].rect.top)) );

  Object pl = lvl.GetObject("player");
  PLAYER Player(anim, lvl, static_cast<int>(pl.rect.left), static_cast<int>(pl.rect.top));

  HealthBar healthBar;

  Clock clock;

  // Главнвый цикл программы
  while (window.isOpen())
    {
      ...
      while (window.pollEvent(event))
        {
          ...
          if (event.type == Event::KeyPressed)
            if (event.key.code == Keyboard::Space)
              entities.push_back(new Bullet(anim2, lvl, static_cast<int>( Player.x+18 ), static_cast<int>( Player.y+15 ), Player.dir) );
        }
          ...
          if ((*it)->Name=="Enemy")
            {
              ...
              if  (Player.getRect().intersects( enemy->getRect() ))
                {
                if (Player.dy>0) { enemy->dx=0; Player.dy=-0.2f; enemy->Health=0;}
